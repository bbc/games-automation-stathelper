export const formatStatConfig = (
    stat,
    { actionName, actionType, campaignId, creationId, variant, format, generalPlacement, detailedPlacement,  advertiserId, url, counterName, screenName, bucket } = {}
  ) => {
    let bucketString;
    ...
    const statString = `[${stat.campaignId}]-[${stat.creationId}]-[${
      stat.variant
    }]-[${stat.format}]-[${setGeneralPlacement(
      stat.generalPlacement,
      stat.screenName
    )}]-[${stat.detailedPlacement}]-[${stat.advertiserId}]-[${stat.url}]`;
    const newStatData = {
      bucket: bucketString,
      stat: statString,
